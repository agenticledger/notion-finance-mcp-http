#!/usr/bin/env node
/**
 * Notion MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Notion API key as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Notion Integration API key as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-notion-api-key>',
      howToGetKey:
        'Create an integration at https://www.notion.so/my-integrations and copy the Internal Integration Secret',
    },
    configTemplate: {
      mcpServers: {
        notion: {
...
<head>
  ...
  <title>Notion Finance MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Notion Finance MCP Server</h1>
    <p class="subtitle">Access Notion pages, databases, blocks, users, search, and comments through MCP tools.</p>

    <div class="card">
      <h2>How to get your Notion API key</h2>
      <ol class="steps">
        <li>Go to <a href="https://www.notion.so/my-integrations" target="_blank" rel="noopener">Notion Integrations</a>.</li>
        ...
        <li>Share the pages/databases you want to access with your integration.</li>
      </ol>
    </div>
...
function createMCPServer(client: NotionClient): Server {
  const server = new Server(
    { name: 'notion-finance-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
<head>
  ...
  <title>Authorize \u2014 Notion Finance MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Notion Finance MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Notion Finance MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Notion Finance MCP HTTP Server running on port ${PORT}`);
