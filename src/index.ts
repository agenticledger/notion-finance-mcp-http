#!/usr/bin/env node
/**
 * Notion MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Notion secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which is the registered Notion
 * OAuth app, owns the client_id/secret, runs the consent flow, and vaults each
 * user's token. See broker-client.ts for the full design.
 *
 * Credential resolution (per request):
 *   1. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for provider=notion -> { accessToken },
 *      call the Notion API directly. If not connected yet, return a
 ...
 *      `Authorization: Bearer <raw-notion-api-key>`, it is used directly.
 *
...
/** Raw passthrough escape hatch — holds no secret. Returns a raw Notion API key. */
...
app.get('/', (_req, res) => {
  res.json({
    name: 'Notion Finance MCP Server',
    provider: 'AgenticLedger',
    version: '2.0.0',
    description:
      'Access Notion pages, databases, blocks, users, search, and comments through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description:
          'Escape hatch (no secret held): pass a raw Notion integration API key as Bearer.',
      },
    },
    configTemplate: {
      mcpServers: {
    ...
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/notion/',
    },
  });
});
...
app.get('/health', (_req, res) => {
  res.json({
    status: 'ok',
    server: 'notion-finance-mcp-http',
    version: '2.0.0',
    tools: tools.length,
    transport: 'streamable-http',
    ...
    authModes: [
      'broker-first (default): resolves Notion via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <notion-api-key>',
    ],
  });
});
...
/**
 * Resolves a NotionClient for the current caller at tool-call time, or a
 * connect-on-first-call message, or an error. Bound per session.
 */
type ClientResolution =
  | { kind: 'client'; client: NotionClient }
...
function createMCPServer(resolveClient: ClientResolver): Server {
  const server = new Server(
    { name: 'notion-finance-mcp-server', version: '2.0.0' },
    { capabilities: { tools: {} } }
  );
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Notion isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        'Notion is not connected for this caller yet. Open the connect link below once (sign in to Notion and grant access), then run the tool again — it will work.',
...
app.post('/mcp', async (req, res) => {
  ...
  if (raw) {
    const client = new NotionClient(raw);
    ...
    resolveClient = async () => {
      ...
      if (tok.status === 'connected') {
        return { kind: 'client', client: new NotionClient(tok.accessToken) };
      }
...
// ==================== START ====================
app.listen(PORT, () => {
  console.log(`Notion Finance MCP HTTP Server v2.0.0 (broker-first)`);
