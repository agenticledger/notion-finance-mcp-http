 ...
 *
 * This MCP holds ZERO Notion/provider secrets. The only secret it carries is a
 ...
 *   3. calls the broker (POST /token) to resolve a ready-to-use Notion access
 *      token (the broker vaults it after consent),
 *   4. calls the Notion API directly with it.
 *
 * When the caller hasn't connected Notion yet, the broker returns 404 and this
 ...
 *
 * This MCP uses the `notion` provider row.
 *
...
const PROVIDER = process.env.BROKER_PROVIDER || 'notion';
...
/**
 * Resolve the caller's Notion access token from the broker.
